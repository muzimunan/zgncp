  ...
  <div class="feekinds">
    <div class="znp-filter-container">
      <el-form ref="formParams" size="small" :inline="true" :model="formParams">
        <el-form-item label="" prop="nameOrCode">
          <el-input v-model="formParams.nameOrCode" placeholder="请输入财务科目名称或编码" clearable />
        </el-form-item>
    ...
    <div class="znp-table-container content-p24">
      <div style="margin-bottom: 16px">
        <el-button v-if="permissions.entity_financekinds_add" type="primary" size="mini" @click="operateData('add')">
          新增
        </el-button>
      </div>
      <el-table
      ...
      >
        <el-table-column prop="financeKindsCode" label="财务科目编码" />
        <el-table-column prop="financeKindsName" label="财务科目名称" />
        <el-table-column prop="label" label="标签名称" />
        <el-table-column prop="originalCode" label="财务原编码" />
        <el-table-column prop="assitanItem" label="辅助核算" />
        <el-table-column prop="remark" label="备注" />
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              v-if="permissions.entity_financekinds_edit"
              size="mini"
              type="text"
            ...
            <el-button
              v-if="permissions.entity_financekinds_view"
              type="text"
              size="mini"
              @click="operateData('view', scope.row)"
            >
              查看
            </el-button>
            <el-button
              v-if="permissions.entity_financekinds_del"
              type="text"
              size="mini"
      ...
      <el-form ref="inputFormRef" :model="inputForm" :rules="inputFormRules" label-width="110px">
        <el-row>
          <el-col :xs="24" :sm="24" :lg="24">
            <el-form-item label="财务科目编码" prop="financeKindsCode" class="znp-remove-start" v-if="!addDisable">
              <el-input
                v-model="inputForm.financeKindsCode"
                placeholder="自动生成，可自定义"
                maxlength="10"
                clearable
                show-word-limit
                :disabled="viewDisable || editDisable"
              />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :lg="24">
            <el-form-item label="财务科目名称" prop="financeKindsName">
              <el-input
                v-model="inputForm.financeKindsName"
                placeholder="请输入财务科目名称"
                maxlength="20"
                clearable
                show-word-limit
          ...
          <el-col :xs="24" :sm="24" :lg="24">
            <el-form-item label="财务原编码" prop="originalCode">
              <el-input
                v-model="inputForm.originalCode"
                placeholder="请输入财务原编码"
                maxlength="20"
                clearable
                show-word-limit
                :disabled="viewDisable"
          ...
          <el-col :xs="24" :sm="24" :lg="24">
            <el-form-item label="辅助核算" prop="assitanItem">
              <el-input
                v-model="inputForm.assitanItem"
                placeholder="请输入辅助核算"
                maxlength="20"
                clearable
                show-word-limit
...
<script>
import { fetchList, addObj, putObj, delObj } from '@/api/feekindsfinance'
import pagination from '@/components/Pagination' // 分页
import { mapGetters } from 'vuex'
export default {
  name: 'FeeKindsOperation',
  components: {
    pagination
  },
  data() {
    ...
    return {
      ...
      inputFormRules: {
        ...
        financeKindsName: [
          { required: true, message: '请输入财务科目名称', trigger: 'blur' }
        ],
        label: [
          { required: true, message: '请输入财务科目标签名称', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    operateData(type, item) {
      ...
      if (this.operateType === 'add') {
        this.dialog.title = '新增财务科目'
        this.inputForm = Object.assign({}, this.baseForm)
      } else if (this.operateType === 'edit') {
        this.dialog.title = '修改财务科目'
        this.inputForm = item
      } else if (this.operateType === 'view') {
        this.dialog.title = '查看财务科目'
        this.inputForm = item
      } else {
        this.$confirm('是否删除财务科目【' + item.financeKindsName + '】信息?', '提示', {
