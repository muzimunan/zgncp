  ...
  <div class="bring-fee-item">
    ...
    >
      <div id="printFeePayDetail">
        <div class="print-fee-pay-title">【{{ feeBackObj.marketName }}】- &nbsp;{{ printDialogVo.title }}</div>
        <div class="nomarl-table-title-descriptions div-desc">
          <span>单据编号:{{ feeBackObj.feebackCode }}</span>
          <span>单据日期:{{ feeBackObj.fillbackTime }}</span>
          <span>商户名称:{{ feeBackObj.merchantName }}</span>
        </div>
        <table class="nomarl-table">
          ...
          <tbody>
            <template v-for="(item, index) in feeBackObj.feebackDetailsPrintResponseList">
        ...
        <div style="text-align: right; color: red" class="div-desc">
          <span>应退总额:{{ feeBackObj.totalAmountRefundable }}</span>
        </div>
        <div style="text-align: left" class="div-desc">
          <span>说明:{{ feeBackObj.remark }}</span>
        </div>
        <div class="nomarl-table-title-descriptions div-desc">
          <span>操作员:{{ feeBackObj.operation }}</span>
          <span>打印时间:{{ feeBackObj.printDate }}</span>
        </div>
      </div>
...
import { putfeeBackPrintObj } from '@/api/feeBack'
export default {
  ...
  data() {
    return {
      ...
      feeBackObj: {
        feebackDetailsPrintResponseList: [],
        merchantName: '', // 商户名称
        marketName: '', // 市场名称
        feebackCode: '', // 收退款单编号
        remark: '', // 描述退款单备注信息
    ...
    handlerSearch() {
      putfeeBackPrintObj({
        ...
        if (res && parseInt(res.data.code) === 0) {
          this.feeBackObj = res.data.data
          this.printObj.popTitle = this.feeBackObj.marketName + '-' + this.printDialogVo.title
        } else {
