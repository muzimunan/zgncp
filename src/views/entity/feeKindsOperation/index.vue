  ...
  <div class="feekinds">
    <div class="znp-filter-container">
      <el-form ref="formParams" size="small" :inline="true" :model="formParams">
        <el-form-item label="" prop="nameOrCode">
          <el-input v-model="formParams.nameOrCode" placeholder="请输入运营科目名称或编码" clearable />
        </el-form-item>
    ...
    <div class="znp-table-container content-p24">
      <div style="margin-bottom: 16px">
        <el-button v-if="permissions.entity_operationkinds_add" type="primary" size="mini" @click="operateData('add')">
          新增
        </el-button>
      </div>
      <el-table
      ...
      >
        <el-table-column prop="operationKindsCode" label="运营科目编码" />
        <el-table-column prop="operationKindsName" label="运营科目名称" />
        <el-table-column prop="label" label="标签名称" />
        <el-table-column prop="remark" label="备注" />
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              v-if="permissions.entity_operationkinds_edit"
              size="mini"
              type="text"
            ...
            <el-button
              v-if="permissions.entity_operationkinds_view"
              type="text"
              size="mini"
            ...
            <el-button
              v-if="permissions.entity_operationkinds_del"
              type="text"
              size="mini"
      ...
      <el-form ref="inputFormRef" :model="inputForm" :rules="inputFormRules" label-width="110px">
        <el-row>
          <el-col :xs="24" :sm="24" :lg="24">
            <el-form-item label="运营科目编码" prop="operationKindsCode" class="znp-remove-start" v-if="!addDisable">
              <el-input
                v-model="inputForm.operationKindsCode"
          ...
          <el-col :xs="24" :sm="24" :lg="24">
            <el-form-item label="运营科目名称" prop="operationKindsName">
              <el-input
                v-model="inputForm.operationKindsName"
                placeholder="请输入运营科目名称"
                maxlength="20"
                clearable
                show-word-limit
...
<script>
import { fetchList, addObj, putObj, delObj } from '@/api/feekindsoperation'
  ...
  data() {
    ...
    return {
      ...
      inputFormRules: {
        ...
        operationKindsName: [
          { required: true, message: '请输入运营科目名称', trigger: 'blur' }
        ],
        label: [
          { required: true, message: '请输入运营科目标签名称', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    operateData(type, item) {
      ...
      if (this.operateType === 'add') {
        this.dialog.title = '新增运营科目'
        this.inputForm = Object.assign({}, this.baseForm)
      } else if (this.operateType === 'edit') {
        this.dialog.title = '修改运营科目'
        this.inputForm = item
      } else if (this.operateType === 'view') {
        this.dialog.title = '查看运营科目'
        this.inputForm = item
      } else {
        this.$confirm('是否删除运营科目【' + item.operationKindsName + '】信息?', '提示', {
