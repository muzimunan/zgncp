  ...
  <div id="svg-diagram-show-area" class="scrollXY" ref="field">
    ...
    >
      ...
      <g :transform="scaleStr">
        <rect x="0" y="0" :width="width" :height="height"  @click="reset"  style="fill: none;stroke: rgb(0, 0, 0);stroke-width: 0.868469;stroke-miterlimit: 4;stroke-dasharray: none;stroke-opacity: 0.990826;stroke-width: 5"/>
        <Node
          :node="item"
...
export default {
  ...
  methods: {
    ...
    updateLinkLocation(obj) {
      const item = this.linkList.find((x) => x.id === obj.id)
      item.point.x = obj.x
      item.point.y = obj.y
    },
    findNode(id) {
    ...
    updateNodeLocation(obj) {
      const item = this.nodeList.find((x) => x.id === obj.id)
      item.point.x = obj.x
      item.point.y = obj.y
    },
    clickNode(id) {
    ...
    commitDest(id) {
      const src = this.nodeList.find((x) => x.id === this.selectedNode)
      const dest = this.nodeList.find((x) => x.id === id)
      this.linkList.push({
        id: this.generateID(),
