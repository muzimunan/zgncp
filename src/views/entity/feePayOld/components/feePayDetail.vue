<template>
  <div class="merchant-papers">
        ...
        <template slot-scope="scope">
          <div v-if="!detailDisable">
            ...
            <div v-else>
              ...
              <el-button
                v-if="permissions.entity_feepaydetails_del && scope.row.realPrice <= 0"
                type="text"
                size="mini"
          ...
          <div v-else>
            ...
            <el-button
              v-if="permissions.entity_feepaydetails_view"
              size="mini"
              type="text"
    ...
    <!-- 添加/编辑收费单明细 -->
    <AddFeePayDetail :dialog-vo="dialogVo" @setDialogVisiable="setDialogVisiable" />
    <!-- 通过费用带入 -->
...
import AddFeePayDetail from './addFeePayDetail'// 【添加或者编辑收费单明细】
import BringFeeItem from './bringFeeItem'// 【添加或者编辑收费单明细】
import { mapGetters } from 'vuex'
import { delObj, chargeRegistration } from '@/api/feepaydetails'
export default {
  name: 'FeePayDetail',
  components: { AddFeePayDetail, BringFeeItem },
  props: {
    tableData: {
      type: Array,
      default: () => []
  ...
  methods: {
    openDialog(type, item) {
      item.feePayId = this.feePayVo.id
      if (type === 'add') {
        this.dialogVo = {
          title: '新增收费单明细',
          visible: true,
    ...
    bringbyFeeItem() {
      ...
      this.bringDialogVo.merchantId = this.feePayVo.merchantId
      this.bringDialogVo.feepayId = this.feePayVo.id
    }
  }
}
</script>
